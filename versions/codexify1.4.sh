...
show_main_menu() {
  ...
  menu_item "1" "Proje" "Proje secimi ve genel durum"
  menu_item "2" "Oturum" "Tmux/Codex oturumu islemleri"
  menu_item "3" "Prompt" "Taslak kaydet, gonder, tekrar et"
  menu_item "4" "Izleme" "Cikti, log ve snapshot ekranlari"
  menu_item "5" "Otomasyon" "Periyodik prompt gonderimi"
...
pane_ready_for_input() {
  ...
  pane_snapshot | clean_pane_output | tail -n 35 | grep -Eq '^[[:space:]]*[›>❯»] '
}
...
start_auto_sender() {
  ...
  (
    ...
    while [ "$i" -le "$count" ]; do
      ...
      if ! wait_for_session_ready 120; then
        cycle_waited=120
        printf '[%s] [%s] tur %s icin hazir ekran algilanamadi, zorla gonderim deneniyor\n' "$(timestamp)" "$PROJECT_NAME" "$i" >> "$AUTO_LOG_FILE"
...
show_session_menu() {
  while true; do
    render_dashboard
    echo
    section_title "Oturum Menusu"
    printf '%s\n' "$(separator)"
    echo "  1) Oturumu baslat"
    echo "  2) Oturuma baglan"
    echo "  3) Oturumlari listele"
    echo "  4) Oturumu kapat"
    echo "  5) Full access sec"
    echo "  6) Geri don"
    echo
    case "$(prompt_input "Secim")" in
      1) run_menu_action start_session ;;
      2) run_menu_action attach_session ;;
      3) run_menu_action list_sessions ;;
      4) run_menu_action stop_session ;;
      5) run_menu_action enable_full_permissions ;;
      6) return 0 ;;
      *) warn "Gecersiz secim" ;;
    esac
    pause_screen
...
show_monitor_menu() {
  while true; do
    ...
    echo
    case "$(prompt_input "Secim")" in
      1) run_menu_action tail_output ;;
      2) run_menu_action show_debug_snapshot ;;
      3) run_menu_action show_script_log ;;
      4) run_menu_action show_auto_sender_log ;;
      5) run_menu_action show_status ;;
      6) return 0 ;;
      *) warn "Gecersiz secim" ;;
    esac
    pause_screen
...
show_automation_menu() {
  while true; do
    render_dashboard
    echo
    section_title "Otomasyon Menusu"
    printf '%s\n' "$(separator)"
    ...
    echo
    case "$(prompt_input "Secim")" in
      1) run_menu_action start_auto_sender_interactive ;;
      2) run_menu_action stop_auto_sender ;;
      3) return 0 ;;
      *) warn "Gecersiz secim" ;;
    esac
    pause_screen
...
interactive_loop() {
  ...
  while true; do
    ...
    case "$choice" in
      1|p|P) run_menu_action show_project_menu ;;
      2|o|O) run_menu_action show_session_menu ;;
      3|r|R) run_menu_action show_prompt_menu ;;
      4|i|I) run_menu_action show_monitor_menu ;;
      5|a|A) run_menu_action show_automation_menu ;;
      q|Q)
