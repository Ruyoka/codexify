...
render_dashboard() {
  ...
  if [ -n "${PROJECT_NAME:-}" ] && session_exists; then
    session_text="$(ok 'aktif')"
  else
    session_text="$(muted 'kapali')"
  fi

  if [ -n "${PROJECT_NAME:-}" ] && auto_sender_running; then
    auto_text="$(ok "aktif pid $(cat "$AUTO_PID_FILE")")"
  else
    auto_text="$(muted 'kapali')"
  fi

  if [ -n "${PROJECT_NAME:-}" ] && [ -f "${DRAFT_PROMPT_FILE:-/dev/null}" ]; then
    draft_text="$(ok 'kayitli')"
  else
    draft_text="$(muted 'yok')"
  fi

  printf '%s\n' "$(separator)"
  printf '%s\n' "$(bold 'CODEXIFY 1.2')"
  printf '%s\n' "$(separator)"
  status_line "Proje" "$(printf '%s' "$project_text" | sed 's/\x1b\[[0-9;]*m//g')"
  status_line "Klasor" "${PROJECT_DIR:-yok}"
  status_line "Session" "${SESSION:-yok} / ${session_text}"
  status_line "Taslak" "$(printf '%s' "$draft_text" | sed 's/\x1b\[[0-9;]*m//g')"
  status_line "Auto" "$(printf '%s' "$auto_text" | sed 's/\x1b\[[0-9;]*m//g')"
  if [ -n "${PROJECT_NAME:-}" ]; then
    status_line "todo/changelog" "todo=$(project_file_status todo.md) changelog=$(project_file_status CHANGELOG.md)"
  fi
  printf '%s\n' "$(separator)"
  echo "p) Proje   o) Oturum   r) Prompt   i) Izleme   a) Otomasyon   q) Cikis"
}

show_main_menu() {
  render_dashboard
  echo
  echo "$(muted 'Ana menu kisadir. Alt menuler numaralidir.')"
  echo
}
...
pane_ready_for_input() {
  ...
  pane_snapshot | clean_pane_output | tail -n 25 | grep -q '^› '
}
...
start_auto_sender() {
  ...
  (
    ...
    while [ "$i" -le "$count" ]; do
      ...
      cycle_waited=0
      until pane_ready_for_input; do
        sleep 2
        cycle_waited=$((cycle_waited + 2))
        if ! tmux has-session -t "$SESSION" 2>/dev/null; then
          printf '[%s] [%s] session kapandi, auto sender durdu\n' "$(timestamp)" "$PROJECT_NAME" >> "$AUTO_LOG_FILE"
          log WARN "AUTO_STOP session_closed_during_wait"
          exit 0
        fi
        if [ $((cycle_waited % 30)) -eq 0 ]; then
          printf '[%s] [%s] tur %s icin uygun an bekleniyor (%ss)\n' "$(timestamp)" "$PROJECT_NAME" "$i" "$cycle_waited" >> "$AUTO_LOG_FILE"
        fi
      done
...
show_project_menu() {
  while true; do
    render_dashboard
    echo
    echo "$(bold 'Proje Menusu')"
    echo "  1) Proje sec"
    echo "  2) Durumu yenile"
    echo "  3) Geri don"
    echo
    case "$(prompt_input "Secim")" in
      1) choose_project_interactive ;;
      2) show_status ;;
      3) return 0 ;;
      *) warn "Gecersiz secim" ;;
    esac
...
show_session_menu() {
  while true; do
    render_dashboard
    echo
    echo "$(bold 'Oturum Menusu')"
    echo "  1) Oturumu baslat"
    echo "  2) Oturuma baglan"
    echo "  3) Oturumlari listele"
...
show_prompt_menu() {
  while true; do
    ...
    echo
    case "$(prompt_input "Secim")" in
      1) save_default_prompt_draft ;;
      2) save_review_prompt_draft ;;
      3) save_test_prompt_draft ;;
      4) save_custom_prompt_draft ;;
      5) send_draft_prompt ;;
      6) send_last_prompt ;;
      7) show_prompt_draft ;;
      8) show_last_prompt ;;
      9) clear_prompt_draft ;;
      10) send_status_command ;;
      11) return 0 ;;
      *) warn "Gecersiz secim" ;;
    esac
    pause_screen
...
show_monitor_menu() {
  while true; do
    render_dashboard
    echo
    echo "$(bold 'Izleme Menusu')"
    echo "  1) Son ciktiyi goster"
    echo "  2) Snapshot goster"
    echo "  3) Script logunu goster"
    echo "  4) Auto sender logunu goster"
    echo "  5) Durum ekrani"
    echo "  6) Geri don"
    echo
    case "$(prompt_input "Secim")" in
      1) tail_output ;;
      2) show_debug_snapshot ;;
      3) show_script_log ;;
      4) show_auto_sender_log ;;
      5) show_status ;;
      6) return 0 ;;
      *) warn "Gecersiz secim" ;;
    esac
...
show_automation_menu() {
  while true; do
    render_dashboard
    echo
    echo "$(bold 'Otomasyon Menusu')"
    echo "  1) Otomatik gondericiyi baslat"
    echo "  2) Otomatik gondericiyi durdur"
    echo "  3) Geri don"
    echo
    case "$(prompt_input "Secim")" in
      1) start_auto_sender_interactive ;;
      2) stop_auto_sender ;;
      3) return 0 ;;
      *) warn "Gecersiz secim" ;;
    esac
...
interactive_loop() {
  ...
  while true; do
    ...
    case "$choice" in
      p|P) show_project_menu ;;
      o|O) show_session_menu ;;
      r|R) show_prompt_menu ;;
      i|I) show_monitor_menu ;;
      a|A) show_automation_menu ;;
      q|Q)
